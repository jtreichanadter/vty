...
def parseDoscar( *Directory ):
    ...
    doscar = { key:None for key in doscarKeyList() }


    with open(fileName,'r') as doscarFile:
...
def warning(*argv):
    print("\033[1;33m--- WARNING : parseDOSCAR.py ---\033[0;33m")
    for arg in argv:
        print(arg)
    print("\033[0m")
...
def isDictionaryDoscar( dictionary ):
    return \
        not any( not key in doscarKeyList() for key in dictionary.keys() ) \
        and \
        not any( not key in dictionary.keys() for key in doscarKeyList() )
...
def printDoscar( dictionary ):
    # check that we are looking at a POSCAR dictionary
    assert isDictionaryDoscar( dictionary ) , "'printDoscar()' READ AN INAPPROPRIATE DICTIONARY"
...
def printFullDoscar( dictionary ):
    # check that we are looking at an POSCAR dictionary
    assert isDictionaryDoscar( dictionary ) , "'printFullDoscar()' READ AN INAPPROPRIATE DICTIONARY"
    # prints everything (including empty keys)
    for item in dictionary.items():
            if isinstance(item[1],list):
...
def printBriefDoscar( dictionary ):
    # check that we are looking at a POSCAR dictionary
    assert isDictionaryDoscar( dictionary )
    # prints everything (including empty keys)
    for item in dictionary.items():
        if item[1]==None:
