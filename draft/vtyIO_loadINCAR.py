#!/usr/bin/env python
def loadINCAR( *filePath ):
    """Loads VASP INCAR file into a python dictionary.
    [0] Abstract:
        Information is parsed from the INCAR file path via the argument.
        There are two ways to use this function:
        (a) Within the pythonic environment.
        (b) As a sub-function within some broader script.
    [1] Dependencies:
            vty/_bash_lib/readArguments.py
            import sys
    [2] Input:
            INCAR dictionary (optional)
    [3] Output:
            INCAR dictionary
    """
    ...
    except IOError: warning("Evidently there is no INCAR at filePath '{0}'".format(filePath))
...
def getTypeIncar( dictionary ):
    ...
    if not isDictionaryIncar( dictionary ):
        warning("getTypeIncar() ---","This doesn't appear to be a valid INCAR dictionary","-- May not have been accessed by processINCAR()?")
...
def warning(*argv):
    print("\033[1;33m--- WARNING : loadIncar ---\033[0;33m")
    for arg in argv:
        print(arg)
    print("\033[0m")
    ...
    for key,value in dictionary.items():
        if not value == None:
            ...
            if key=="SYSTEM":
                ...
                # Gives user some context for the file.
                continue
...
if __name__ == '__main__':
    print("The 'loadINCAR()' function is being run as a stand-alone script right now!")
    INCAR = loadINCAR( '3scf' )
    getTypeIncar( INCAR )
