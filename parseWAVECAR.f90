...
! compute norm
waveNorm = sqrt( dot_product(coeff(1:nplane),coeff(1:nplane)) )
...
! CASE: create default unit-cell grid
  ...
  iYsample = 2*nb2max
  iZsample = 2*nb3max
  iSampleCount=iXsample*iYsample*iZsample
  ! allocate memory
  ...
  do ix=1,iXsample
    i=i+1
    sampling(i,1) = dble(ix-1) / dble(iXsample)
    sampling(i,2) = dble(iy-1) / dble(iYsample)
    sampling(i,3) = dble(iz-1) / dble(iZsample)
  enddo
  enddo
  enddo
